...
class InverseKinematics(Node):
    ...
    def __init__(self):
        ...
        self.ik_timer_period = 1.0 / 50   # 10 Hz
    ...
    def inverse_kinematics(self, target_ee, initial_guess=[0, 0, 0]):
        def cost_function(theta):
            ...
            ###########ite-packages/rclpy/publisher.py", line 70, in publish#####################################################################################
        ...
        def gradient(theta, epsilon=1e-3):
            ...
            ################################################################################################
            gradient_vector = np.zeros(theta.shape)

            for i in range(theta.shape[0]):
                epsilon_vector = np.zeros(theta.shape)
                epsilon_vector[i] = epsilon

                gradient_i = (cost_function(theta + epsilon_vector)[0] - cost_function(theta - epsilon_vector)[0]) / (epsilon * 2)

                gradient_vector[i] = gradient_i

            return gradient_vector
        
        theta = np.array(initial_guess)
        learning_rate = 10  # The learning rate
        max_iterations = 20 # The maximum number of iterations
        tolerance = 1 / 5000 # One mm: the tolerance for the L1 norm of the error
        ...
        for _ in range(max_iterations):
            ...
            cost, displacement = cost_function(theta)
